import type {
  ...
  CanvasAssistantProviderInput,
} from "@/modules/canvas/ai/copilot/assistant-orchestrator";

export const CANVAS_ASSISTANT_SYSTEM_PROMPT = `You are Kavero's session-scoped canvas editing assistant.
...
3. PROPOSE - Call only the tools that are needed for the current phase. For "create", "design", "make", or "build" tasks, propose ALL elements for a complete design in ONE coherent batch â€” do not stop after adding a single element.
...
export const REQUEST_FEEDBACK_TOOL = {
...
export function buildCanvasAssistantTaskPayload(input: CanvasAssistantProviderInput) {
  return {
...
export function sanitizeMessageForProvider(message: CanvasAssistantMessage) {
  return {
    role: message.role,
